...
namespace faucets {
  // CONTRACTS
//   inline constexpr auto addsubscriber_ricardian = R"(---
// spec_version: 0.0.1
// title: Add a new subscriber
// summary: The intent of the `{{ addsubscriber }}` action is to save a record of the information inmutability stored in a hash format.
// ---)";

  // CLAUSES
  inline constexpr auto datastorage_clause = R"(The faucet application values the security of personal data. We may process only minimal user data as much as it is necessary to maintain the faucet application running. We only receive and store any information you knowingly provide to us when you create an account, or fill using the app.)";

  inline constexpr auto datausage_clause = R"(This smart contract will process user data to keep the faucet application running. Any of the information we collect from you may be used for these purposes:
- Create and manage user accounts.
- Run and operate the faucet application.)";

  inline constexpr auto dataownership_clause = R"(The user of this smart contract verifies that the smart contract owns the data and that it can use the data in accordance with the terms defined in the Ricardian contract.
...
We will retain and use your personal information for the period necessary to comply with our legal obligations, resolve disputes, and enforce our agreements unless a more extended retention period is required or permitted by law. We may use any aggregated data derived from or incorporating your personal information after you update or delete it, but not in a manner that would identify you personally. Once the retention period expires, personal information shall be deleted.)";
  ...
  inline constexpr auto datadistribution_clause = R"(The smart contract promises not actively to share or distribute the address data. The smart contract user understands that data stored in a multi-index table is not private and can be accessed by any user of the blockchain.
